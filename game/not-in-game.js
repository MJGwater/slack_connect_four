...
const helpers = require('./not-in-game-helpers');
...
hears('^play <@([a-z0-9-._]+)>', 'direct_message', (bot, message) => {
  ...
  if (player1 === player2) {
    ...
    teams.get(message.team, (err, teamData) => {
      const playingGame = helpers.checkIfPlayingGame(player1, player2, teamData);
...
hears('^(?!^play (<@[a-z0-9-._]+>))', 'direct_message', (bot, message) => {
  teams.get(message.team, (err2, teamData) => {
    const playingGame = helpers.checkIfPlayingGame(message.user, undefined, teamData);
    if (playingGame === 'player 1' && (Number(message.text) >= 1 && Number(message.text) <= 7)) {
      ...
      bot.startConversation(message, (err, convo) => {
        ...
        helpers.startGameInstructions(convo);
      });
    }
  });
});
